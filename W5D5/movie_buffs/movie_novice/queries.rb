...
def pulp_fiction_actors
  ...
  Actor.select(:name, :id).join(:casting).join(:movie).where(movie: 'Pulp Fiction')
...
def uma_movies
  ...
  Movie.select(:title, :id, :yr).join(:casting).join(:actor).where(name: 'Uma Thurman').order('yr ASC')
end
